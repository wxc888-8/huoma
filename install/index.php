...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">短网址系统</h3>
	</div>
...
<table class="table table-striped">
	...
	<tbody>
		<tr>
			<td>php5.3 - 7.4</td>
			<td>必须</td>
		<!--	<td><?php echo version_compare(PHP_VERSION, '5.3.0', '>'&& '7.4.0', '<')?'<font color="green">'.PHP_VERSION.'</font>':'<font color="red">'.PHP_VERSION.'</font>'; ?></td>
			<td>PHP版本支持</td>
		</tr>-->
		<td><?php echo '<font color="red">'.$phpv.'</font>';?></td>
		<?php
	          if ($phpv >= '5.5' && $phpv < '7.4') {
	            echo '<td><span class="badge badge-pill badge-success">支持</span></td>';
	          }else{
	            echo '<td><span class="badge badge-pill badge-danger">不支持</span></td>';
	          }
	        ?>
	      </tr>
		<tr>
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from ".$dbconfig['dbqz']."_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过短网址系统</div>
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	...
	$date = date("Y-m-d");
	DB::query("INSERT INTO `dwz_config` VALUES ('build', '".$date."')");
	DB::query("INSERT INTO `dwz_config` VALUES ('cronkey', '".rand(100000,999999)."')");
	DB::query("INSERT INTO `dwz_config` VALUES ('syskey', '".random(32)."')");
}
if($e==0) {
