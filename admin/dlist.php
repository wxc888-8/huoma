...
<body>
            ...
            <div class="col-lg-12">
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h6 class="modal-title" id="addChangeTitle">新增域名</h6>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <form name="add" action="ajax.php?act=addDomain">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="type" id="add-type" required>
                                            <option value="0">通用入口域名</option>
                                            <option value="1">通用落地域名</option>
                                            <option value="2">普通入口域名</option>
                                            <option value="3">普通落地域名</option>
                                            <option value="4">防红入口域名</option>
                                            <option value="5">防红落地域名</option>
                                            <option value="6">直链入口域名</option>
                                            <option value="7">直链落地域名</option>
                                        </select>
                                        <small class="help-block">请不要使用落地域名作为短链接口域名</small>
                                    </div>
                                    <div class="form-group">
                                        <label for="add-http" class="control-label">协议：</label>
                                        <select class="form-control" name="is_https" id="add-http">
                ...
                <div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="editChangeLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h6 class="modal-title" id="editChangeTitle">修改域名信息</h6>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <form name="edit" action="ajax.php?act=editDomain">
                                    ...
                                    <div class="form-group">
                                        <label for="edit-http" class="control-label">协议：</label>
                                        <select class="form-control" name="is_https" id="edit-http">
                                            <option value="0">http</option>
                                            <option value="1">https</option>
                                        </select>
                                    </div>
                ...
                <div class="modal fade" id="batchAdd" tabindex="-1" role="dialog" aria-labelledby="batchAddLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h6 class="modal-title" id="batchAddTitle">批量添加域名</h6>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <form name="batchAdd" action="ajax.php?act=batchAddDomain">
                                    <div class="form-group">
                                        <label for="batch-domains" class="control-label">域名列表：</label>
                                        <textarea class="form-control" name="domains" id="batch-domains" rows="8" placeholder="每行一个域名，例如：example.com" required></textarea>
                                        <small class="help-block text-muted">每行输入一个域名，不需要添加http://或https://前缀</small>
                                    </div>
                                    <div class="form-group">
                                        <label for="batch-type" class="control-label">域名类型：</label>
                                        <select class="form-control" name="type" id="batch-type" required>
                                            <option value="0">通用入口域名</option>
                                            <option value="1">通用落地域名</option>
                                            <option value="2">普通入口域名</option>
                                            <option value="3">普通落地域名</option>
                                        ...
                                        <small class="help-block text-muted">批量添加的所有域名将使用相同的类型</small>
                                    </div>
                                    <div class="form-group">
                                        <label for="batch-http" class="control-label">协议：</label>
                                        <select class="form-control" name="is_https" id="batch-http">
                                            <option value="0">http</option>
                                            <option value="1">https</option>
                                        </select>
                                        <small class="help-block text-muted">批量添加的所有域名将使用相同的协议</small>
                                    </div>
                                </form>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">关闭</button>
                                <button type="submit" class="btn btn-primary ajax-post refresh" target-form="batchAdd">批量添加</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div id="toolbar" class="toolbar-btn-action">
                            ...
                            <button id="btn_batch_add" type="button" class="btn btn-info" data-toggle="modal" data-target="#batchAdd">
                                <span class="mdi mdi-playlist-plus" aria-hidden="true"></span>批量添加
                            </button>
    ...
    <script type="text/javascript">
            ...
            $('#listTable').bootstrapTable({
                ...
                columns: [{
                        ...
                        formatter: function(value, row, index) {
                            switch (row.type) {
                                ...
                                case '7':
                                    ...
                                    break;
                                case '8':
                                    type = '<font color="blue">短链接口域名</font>';
                                    break;
                                default:
                                    type = '未知域名';
                                    break;
                    ...
                    },
                    {
                        ...
                        title: '协议',
                        formatter: function(value, row, index) {
                            var value = row['is_https'] == 1 ? 'https' : 'http';
                            return value;
                        },
                        sortable: true
                    },
        ...
        function setactive(id, state) {
            ...
            $.ajax({
                type: 'GET',
                url: 'ajax.php?act=setDomainState&id=' + id + '&state=' + state,
                dataType: 'json',
                ...
                buttons: {
                    confirm: {
                        ...
                        action: function() {
                            ...
                            $.ajax({
                                url: "ajax.php?act=setDomainStateAll",
                                type: "POST",
                                data: {
                                    "state": state,
                                    "str": postData
        ...
        $('#edit').on('show.bs.modal', function(event) {
            ...
            $.ajax({
                type: 'GET',
                url: 'ajax.php?act=getDomainInfo&id=' + id,
                dataType: 'json',
                success: function(data) {
                    loader.destroy();
                    if (data.code == 0) {
                        ...
                        modal.find('#edit-http').val(data.is_https)
                    } else {
        ...
        function checkdomain(id, type) {
            ...
            $.ajax({
                type: 'GET',
                url: 'ajax.php?act=checkDomain&id=' + id + '&type=' + type,
                dataType: 'json',
                success: function(data) {
                    loader.destroy();
                    if (data.code == 0) {
